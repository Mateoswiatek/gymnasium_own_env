...
### Mechanizm ruchu armii

Gracze mogą inicjować przemieszczenie jednostek między miastami poprzez wydanie rozkazu marszu określonej liczby wojowników z miasta źródłowego do miasta docelowego. Czas podróży armii jest proporcjonalny do euklidesowej odległości między punktami, co wprowadza element taktyczny związany z czasowaniem ataków oraz koordynacją wielofrontowych operacji. System ten wymaga od graczy antycypacji ruchów przeciwnika oraz strategicznego rozplanowania sekwencji działań.

### System walki i podboju

Armie przemieszczające się po identycznych trasach w przeciwnych kierunkach mogą wejść w bezpośredni konflikt, rezultując wzajemnym niszczeniem jednostek w stosunku jeden do jednego. Mechanizm ten wprowadza element taktyczny związany z blokowaniem tras komunikacyjnych oraz prowadzeniem wojny pozycyjnej. Po dotarciu do miasta docelowego armie wykonują jedną z dwóch akcji: w przypadku miasta przyjaznego jednostki zostają włączone do lokalnej garnizonu, wzmacniając obronę pozycji; w przypadku miasta wrogiego następuje bezpośrednie starcie, w którym przewaga liczebna decyduje o sukcesie operacji. Miasto zostaje podbite gdy atakujące siły przewyższają liczebnie obronę, przejmując kontrolę nad pozycją oraz pozostawiając nadwyżkę jednostek jako nowy garnizon.

## Architektura techniczna

### Framework środowiska

System został zaimplementowany w oparciu o bibliotekę Gymnasium, będącą standardem w dziedzinie uczenia ze wzmocnieniem. Implementacja przestrzega interfejsu OpenAI Gym, zapewniając kompatybilność z szerokimi spektrum algorytmów uczenia maszynowego oraz narzędzi badawczych. Środowisko udostępnia standardowe metody reset(), step(), render() oraz właściwości action_space i observation_space, umożliwiając bezproblemową integrację z istniejącymi frameworkami uczenia ze wzmocnieniem.
...
Przestrzeń obserwacji została skonstruowana jako złożona struktura słownikowa zawierająca kompletne informacje o stanie gry. Komponenty obserwacji obejmują rozkład jednostek w poszczególnych miastach, informacje o właścicielach wszystkich pozycji na planszy oraz szczegółowe dane dotyczące armii obecnie przemieszczających się między miastami. Każda armia w ruchu charakteryzowana jest przez miasto źródłowe, cel podróży, przynależność do gracza, liczbę jednostek, postęp podróży oraz całkowity dystans do pokonania.

### System wizualizacji

Komponent wizualizacji wykorzystuje bibliotekę PyGame do renderowania stanu gry w czasie rzeczywistym. Miasta przedstawiane są jako kolorowe okręgi, których barwa odpowiada przynależności do gracza, z wyraźnie oznaczoną liczbą stacjonujących jednostek. Połączenia między miastami wizualizowane są jako linie z etykietami dystansu, ułatwiając graczom planowanie strategiczne. Armie w ruchu reprezentowane są jako animowane punkty poruszające się wzdłuż tras, z rozmiarem proporcjonalnym do liczby transportowanych jednostek.
